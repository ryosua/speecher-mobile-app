...
function scene:create( event )
    ...
    composer.state.returnTo = "src.view.scenes.notecardView"

    local notecard = event.params.notecard

    -- Set up the controllers.
    crudControllerI = singletonsI.getNoteControllerI(notecard)
    ...
    topBarI.setText(notecard.title .. " > Notes")
    ...
    function onCreateModelButtonTap(e)
        if createModelButtonI.mode == createModelButton.CREATE_MODE then
            return crudGUIController.doOnCreate(crudControllerI, editFieldI, strings.newNoteDefaultText, strings, tableViewI, singletonsI, e)
    ...
    editFieldI.placeholder = strings.newNoteDefaultText
